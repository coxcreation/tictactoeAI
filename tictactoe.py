    ...
    def populateChildMoves(self, gb):
        ...
        for x in range(len(gb)):
            if gb[x] == ' ':
                ...
                game_move = []
                for loc in gb:
                    game_move.append(loc)
                game_move[x] = TicTacToeGame.playerSymbols[self.turn % 2]
                childGame = TicTacToeGame(game_move, next_turn)
                #print("Position: "+str(x)+" Board:"+str(game_move))
